...
// ── Rental Terms (Apartment/Condo) ──────────────────────────────────────
// Moved to rental-terms.js — the dedicated Rental Terms domain module
// (field registry, display facts/summary, Estimated Move-in Cost, filter
// predicates, and inherit/override resolution). Load rental-terms.js right
// after this file; resolveUnitType() below calls into it
// (pickRentalTermsColumns()) to resolve a unit type's Rental Terms
// overrides, and rental-terms.js calls back into this file's
// resolveFieldDisplayValue() and resolveUnitType() — both files must be
// present together on any page that touches Unit Types or Rental Terms.
// See rental-terms.js's file header for the full picture, and prefer its
// `RentalTerms` namespace object over ad-hoc new logic here.
...
function resolveUnitType(property, unitType) {
  ...
  return {
    ...
    // delegates to rental-terms.js's pickRentalTermsColumns(), passing this
    // function's own pick() closure, so the Rental Terms column list lives
    // in exactly one place (that module's RENTAL_TERMS_FIELDS) rather than
    // being walked a second time here. rental-terms.js must be loaded
    // alongside this file wherever Unit Types are resolved.
    rentalTerms: (typeof pickRentalTermsColumns === 'function') ? pickRentalTermsColumns(pick) : {}
  };
}

// getResolvedRentalTermsRow(property, unitType) — the Rental Terms-shaped
// convenience built on top of resolveUnitType() above (camelCase fields ->
// a plain row-shaped object getRentalTermsFacts()/getEstimatedMoveInCost()/
// getRentalTermsSummary() can consume directly) — now lives in
// rental-terms.js (also reachable as RentalTerms.getResolvedRow()), the
// canonical home for Rental Terms display/computation logic. Load
// rental-terms.js to use it.
